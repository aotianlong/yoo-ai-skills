...
在运行 `gh issue create` 之前，根据 Step V1～V2 整理好标题、标签与 body；**信息足够则直接创建**，可在同一条回复里简要复述标题与标签。**不必**先完整展示草案再等一句「确认」。

若用户**明确要求**先发草案审阅，或需求仍有关键缺口须追问，再停顿对齐后再 `gh issue create`。
...
若用户事先要求「给我看全文再建」，则在对话中展示标题、body、标签并等待；否则不要为确认而阻断流程。

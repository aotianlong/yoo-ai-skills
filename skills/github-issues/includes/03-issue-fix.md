...
> **解决前**：若该 issue 下已有 **`## 🔍 Issue 解读`**（解读贴），**先通读其中「可执行修复方案」** 再改代码；解读贴即**方案与调整的载体**，与人有不同意见时在 issue 评论里跟进。**默认**按最新解读贴实现；若实现中发现方案必须偏离，在 PR/issue 评论简短说明原因。
...
用户在**同一任务**中明示 **只要解读**、**只发帖**、**不要改代码**、**别开分支** 等时：**只**执行 `02-issue-interpretation.md` Step R1～R6（或批量脚本），**禁止** `git checkout -b`、改业务代码、`git commit`、开 PR。

若用户说 **先解读再解决** 且**未**排除写代码：先发解读贴，再按上文「默认：直接全量实现」继续。

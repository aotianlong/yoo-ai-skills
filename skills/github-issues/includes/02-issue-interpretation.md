...
- **预览与修订**：解读评论发出后，人类若要调整方案应在 **issue 评论**里说明；你下次执行任务时以 issue 上**最新共识**为准。**不要求**在聊天里先征得「可以开工」再编码——那是「解决」流程的职责（见 `03-issue-fix.md`）。
...
> **发帖要求**（针对仍打开的 issue）：完成分析后，**默认立即**用 `gh issue comment`（或等价脚本）发布 **Step R5** 模板全文。仅在用户**明确要求**先看草案再发帖，或改动 GitHub 的风险很大（例如批量覆盖）时，再在对话中展示全文征求意见。不能仅在对话中给摘要却从不发帖（除非用户取消发帖）。
...
> **此步骤为必须执行步骤**（在仍打开且需发帖的前提下），不可跳过。分析完成后，按下方模板生成完整正文（`## 🔍 Issue 解读` 起至结尾），**默认立即**执行 `gh issue comment`。仅在用户事先要求「先发我看」或正文可能引起大范围误解时，再在发帖前于对话中展示全文。
...
> 以下步骤应具体到**能开 PR 的程度**，便于 AI **直接按此修改**；若方案有误，在 issue 评论里修订或在解读贴下追加说明。
...
评论发布成功后，将 **issue 链接与解读评论锚点** 告知用户；后续「解决」按解读贴与 issue 最新评论执行（见 `03`）。
...
当之后进入 **Issue 解决流程** 时，实现方应 **优先阅读** **`## 🔍 Issue 解读`** 中的 **「可执行修复方案」**；若有出入以 issue 上**最新评论与解读贴**为准（见 `03`）。
...
运行前可在对话中用一两句说明**将发帖的 issue 编号范围**（可选附一条样例路径）；**默认可直接执行**下述命令，无需等待用户回复「可以」。若用户事先要求先看列表再跑脚本，再停顿展示。
...
可在对话中简述将影响的 issue 范围；**默认可直接执行**（建议首次用 `--dry-run` 自检）。若用户要求先看范围再跑，再停顿对齐。

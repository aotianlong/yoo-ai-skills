...
> **发帖要求**（针对仍打开的 issue）：完成分析后，**默认直接用** `gh issue comment`（或等价脚本）发布 **Step R5** 模板全文；**不必**先在对话里贴完整预览再等用户说「发吧」。若用户**明确要求**先看草案再发帖，或改动 GitHub 的风险很大（例如批量覆盖），再在对话中展示全文征求意见。不能仅在对话中给摘要却从不发帖（除非用户取消发帖）。
...
评论发布成功后，将 **issue 链接与解读评论锚点** 告知用户，并说明：**可在发起「请按 issue 解决」类任务前先打开该评论预览方案**。
...
当之后进入 **Issue 解决流程** 时，实现方（人或 AI）应 **优先阅读** 该 issue 下 **`## 🔍 Issue 解读`** 中的 **「可执行修复方案」**；若与当前代码已有出入，以 issue 上最新讨论为准。

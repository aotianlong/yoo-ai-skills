...
1. 根据用户意图**精读上表中对应的 include 文件**，不必默认读完所有分片。用户只要**罗列或筛选** issues 时，打开 **`07-list-issues.md`**。
...
6. **解决 issue 后**：按 `03` 完成 **push + `gh pr create`（base `dev`）**；在 issue 评论中写总结并附 PR 链接。**不得**在仅本地提交后结束任务（无法推送时须说明原因）。**默认**用 **`git worktree`** 在 **`.worktrees/wt-{issue-id}`** 内编码；PR 创建成功后 **`git worktree remove`** 清理。用户说不用 worktree 时跳过此模式（细则见 `03`「Worktree 模式」）。

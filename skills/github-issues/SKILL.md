...
6. **修复 issue 后**：按 `03` 完成 **push + `gh pr create`（base `dev`）**；在 issue 评论中写总结并附 PR 链接。**不得**在仅本地提交后结束任务（无法推送时须说明原因）。
